...
if __name__ == "__main__":
    ...
    caffeNetwork = cv2.dnn.readNetFromCaffe(
        "assets/file/SSD_MobileNet_prototxt.txt", "assets/file/SSD_MobileNet.caffemodel")
    cap = cv2.VideoCapture("assets/videos/pedestrians.mp4")
    fourcc = cv2.VideoWriter_fourcc(*"XVID")
    output_movie = cv2.VideoWriter("assets/videos/result.avi", fourcc, 24, (int(
    ...
    while cap.isOpened():
        ...
        pDetection = cv2.dnn.blobFromImage(cv2.resize(
            frame, (imageWidth, imageHeight)), 0.007843, (imageWidth, imageHeight), 127.5)
        ...
        cv2.putText(frame, "Person in High Risk : " + str(len(highRisk)),
                    (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.putText(frame, "Person in Medium Risk : " + str(len(mediumRisk)),
                    (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        cv2.putText(frame, "Detected Person : " + str(len(detectionCoordinates)),
                    (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        drawResult(frame, position)
        if write_video:
            output_movie.write(frame)
        cv2.imshow('Result by Video', frame)
